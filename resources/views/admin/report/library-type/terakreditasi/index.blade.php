...
<div class="card border-0 shadow mb-4">
    <div class="card-header">
        <h2 class="card-title">Report Jumlah Perpustakaan Terakreditasi Berdasarkan Jenis Perpustakaan Dalam Tahun</h2>
            ...
            <table id="datatable" class="table table-centered table-nowrap mb-0 rounded">
                ...
                <tbody>
                    @foreach ($fetchData['data']['year_data'] as $key => $value)
                        <tr>
                            <td><a href="{{ route('admin.report.library_type.terakreditasi_in_year.show', ['category' => $value['category']]) }}">{{ $value['category'] }}</a></td>
                            @foreach ($years as $year)
                                <td>{{ $value[$year] ?? 0 }}</td>
                            @endforeach
                        </tr>
                ...
                <tfoot>
                    <tr>
                        <td>Total</td>
                        @foreach ($fetchData['data']['totals'] as $item)
                            @foreach ($years as $year)
                                <td>{{ $item[$year] ?? 0 }}</td>
                            @endforeach
                        @endforeach
