...
<div class="card border-0 shadow mb-4">
    <div class="card-header">
        <h2 class="card-title">Report Jumlah Akreditasi Berdasarkan Provinsi Dalam Tahun</h2>
        ...
        <div class="table-responsive">
            <table class="table table-centered table-nowrap mb-0 rounded">
                ...
                <thead>
                    <tr>
                        <td>Total</td>
                        @foreach ($fetchData['data']['per_provinces_total'] as $item)
                            @foreach ($years as $year)
                                <td>{{ $item[$year] }}</td>
                            @endforeach
                        @endforeach
...
@push('js')
    <script>
        const url = "{{ route('admin.report.province.index') }}"
