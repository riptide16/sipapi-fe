...
<div class="card border-0 shadow mb-4">
    <div class="card-header">
        <h2 class="card-title">Report Jumlah Terakreditasi Berdasarkan Provinsi Dalam Tahun</h2>
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table id="datatable" class="table table-centered table-nowrap mb-0 rounded">
                ...
                <tbody>
                    @foreach ($fetchData['data']['provinces'] as $value)
                        <tr>
                            <td>{{ $value['name'] }}</td>
                            @foreach ($years as $year)
                                <td>{{ $value[$year] ?? 0 }}</td>
                ...
                <tfoot>
                    <tr>
                        ...
                        @foreach ($years as $year)
                            <td>{{ $fetchData['data']['province_totals'][0][$year] ?? 0 }}</td>
                        @endforeach
                    </tr>
                </tfoot>
            </table>
